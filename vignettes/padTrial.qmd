...
## `split = NULL` — whole dataset as one group

When there are no blocking columns, or the entire trial is treated as a
single unit, pass `split = NULL`. The whole data frame is processed as one
group and the bounding box is computed globally.

```{r ex_null_split}
# Single 6×5 block, no Block column needed
ex_ns        <- expand.grid(Row = 1:6, Column = 1:5, KEEP.OUT.ATTRS = FALSE)
ex_ns$Type   <- "DH"
ex_ns$Geno   <- paste0("G", sprintf("%02d", seq_len(nrow(ex_ns))))
# Guard row at top and bottom
ex_ns$Type[ex_ns$Row == 1L | ex_ns$Row == 6L] <- "Guard"
# Remove 2 interior DH plots
ex_ns <- ex_ns[-which(ex_ns$Row == 3L & ex_ns$Column == 2L), ]
ex_ns <- ex_ns[-which(ex_ns$Row == 4L & ex_ns$Column == 4L), ]

res_ns <- padTrial(ex_ns, match = "DH", split = NULL, verbose = TRUE)

cat("'new' rows:", sum(res_ns$add == "new"), "\n")
```

```{r ex_null_plot}
#| fig-height: 4
p_ns <- plot_padTrial(res_ns, data = ex_ns)
print(p_ns)
```

---

## Tile labels — `label` argument

Pass any column name to `label` to print values inside each tile. Labels are
suppressed on padded (`"new"`) tiles automatically. This is useful for
checking that genotype assignments are correct after extraction.

```{r ex_label}
#| fig-height: 4
# Use a small layout so labels are readable
ex_lbl        <- expand.grid(Row = 1:4, Column = 1:5, KEEP.OUT.ATTRS = FALSE)
ex_lbl$Type   <- "DH"
ex_lbl$Block  <- "B1"
ex_lbl$Geno   <- paste0("G", sprintf("%02d", seq_len(nrow(ex_lbl))))
ex_lbl        <- ex_lbl[-which(ex_lbl$Row == 2L & ex_lbl$Column == 3L), ]

res_lbl <- padTrial(ex_lbl, split = "Block", verbose = FALSE)

p_lbl <- plot_padTrial(res_lbl, data = ex_lbl, split = "Block",
                        label = "Geno")
print(p_lbl)
```
...
## `verbose = TRUE` — per-group diagnostics

Setting `verbose = TRUE` prints a message for each group showing the detected
bounding box and how many cells were padded. Useful for a quick sanity check
when processing large multi-block trials.

```{r ex_verbose}
invisible(
  padTrial(ex4, match = "DH", split = "Block",
           fill_value = "Blank", verbose = TRUE)
)
```
